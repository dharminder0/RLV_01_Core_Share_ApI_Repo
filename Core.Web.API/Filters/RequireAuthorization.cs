...
namespace Core.Web.Api.Filters {
    ...
    public class RequireAuthorization : Attribute, IAuthorizationFilter {
        ...
        public void OnAuthorization(AuthorizationFilterContext context) {
            ...
            //var defaultAuthorizationValue = CoreConfigurationManager.AppSettings["AuthorizationBearer"];
            ...
            if (bearerAutorizeKeys != null && bearerAutorizeKeys.Any()) {
                bearerAutorizeKeys.ToList().ForEach(k => {
                    if (!string.IsNullOrWhiteSpace(CoreConfigurationManager.AppSettings[k]))
                        bearerAuthorizeValues.Add(CoreConfigurationManager.AppSettings[k]);
                });
            }
